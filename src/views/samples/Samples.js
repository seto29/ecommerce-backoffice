...
import {getAll, fDelete, fUpdate, fInsert} from '../../services/Samples'
...
const Samples = () => {
  ...
  let number = 0
    ...
    let tableData = samples && samples.map(({ id, cName, name, description, image,cogs, price, stock, sku, cID, sold, rating }) => {
        number++
        const data = {
            no: number,
            id: id,
